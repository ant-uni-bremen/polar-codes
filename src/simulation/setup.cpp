...
{
    vector<string> SimTypesVector = { "single",     "codelength", "designsnr",
                                      "listlength", "rate",       "amplification",
                                      "fixed",      "depthfirst", "scan",
                                      "fastsscan",  "ask",        "compareall",
                                      "getcode" };
...
void Configurator::setupArgumentWorkload()
{
    auto workload =
        new ValueArg<long>("w",
                           "workload",
                           "Set the number of bits per simulation run (default: 4e8)",
                           false,
...
{
    auto snrMin = new ValueArg<float>("",
                                      "snr-min",
                                      "Set the lower SNR (Eb/No) in dB.",
                                      false,
                                      defaultFloats["snr-min"],
                                      "float");
    auto snrMax = new ValueArg<float>("",
                                      "snr-max",
                                      "Set the upper SNR (Eb/No) in dB.",
                                      false,
                                      defaultFloats["snr-max"],
                                      "float");
    auto snrCount = new ValueArg<int>(
        "",
        "snr-count",
        "Set the number of SNRs (and thus simulation runs per configuration).",
        false,
        defaultInts["snr-count"],
        "integer");
...
void Configurator::setupArgumentDesignSnr()
{
    auto dsnrFixed = new ValueArg<float>(
        "d",
        "design-snr",
        "Set the fixed design-SNR parameter for Bhattacharrya code construction",
        false,
        defaultFloats["dsnr-fixed"],
        "float");
    auto dsnrMin = new ValueArg<float>("",
                                       "dsnr-min",
                                       "Set the lower design-SNR.",
                                       false,
                                       defaultFloats["dsnr-min"],
                                       "float");
    auto dsnrMax = new ValueArg<float>("",
                                       "dsnr-max",
                                       "Set the upper design-SNR.",
                                       false,
                                       defaultFloats["dsnr-max"],
                                       "float");
    auto dsnrCount = new ValueArg<int>("",
                                       "dsnr-count",
                                       "Set the number of design-SNRs.",
                                       false,
                                       defaultInts["dsnr-count"],
                                       "integer");
...
void Configurator::setupArgumentBlockLength()
{
    auto nFixed = new ValueArg<int>("n",
                                    ...
                                    defaultInts["n-fixed"],
                                    "int");
    auto nMin = new ValueArg<int>(
        "", "n-min", "Minimum blocklength.", false, defaultInts["n-min"], "int");
    auto nMax = new ValueArg<int>(
        "", "n-max", "Maximum blocklength", false, defaultInts["n-max"], "int");
...
{
    auto rFixed = new ValueArg<float>("r",
                                      "rate",
                                      "Set the fixed code rate.",
                                      false,
                                      defaultFloats["r-fixed"],
                                      "float");
    auto rMin = new ValueArg<float>(
        "", "r-min", "Set the lower code rate.", false, defaultFloats["r-min"], "float");
    auto rMax = new ValueArg<float>(
        "", "r-max", "Set the upper code rate.", false, defaultFloats["r-max"], "float");
    auto rCount = new ValueArg<int>("",
                                    "r-count",
                                    "Set the number of code rates.",
                                    false,
                                    defaultInts["r-count"],
                                    "integer");
...
void Configurator::setupArgumentListLength()
{
    auto lFixed =
        new ValueArg<int>("l",
                          "pathlimit",
                          "For list decoding, set the maximum number of parallel paths.",
                          false,
                          defaultInts["l-fixed"],
                          "int");
    auto lMin = new ValueArg<int>(
        "", "l-min", "Minimum path limit", false, defaultInts["l-min"], "int");
    auto lMax = new ValueArg<int>(
        "", "l-max", "Maximum path limit", false, defaultInts["l-max"], "int");
...
{
    vector<string> ErrDetTypesVector = {
        "none", "crc8", "crc32", "cmac8", "cmac16", "cmac32", "cmac64", "cmac128",
    };
    availableErrDets = new ValuesConstraint<string>(ErrDetTypesVector);

    auto ErrDet =
        new ValueArg<string>("e",
                             "error-detection",
...
{
    auto Systematic = new SwitchArg("s",
                                    "non-systematic",
                                    "Disable systematic polar coding.",
                                    defaultBools["non-systematic"]);
...
void Configurator::setupArgumentDecodingPrecision()
{
    auto Precision =
        new ValueArg<int>("p",
                          "precision",
                          "Select decoding precision (32-bit floating point, 8-bit fixed "
                          "integer or '832' for mixed precision).",
                          false,
                          defaultInts["precision"],
                          "8,32,832");
...
{
    auto ampFixed = new ValueArg<float>(
        "a",
        "amplification",
        "Set the fixed amplification factor for 8-bit pre-quantization scaling.",
        false,
        defaultFloats["amp-fixed"],
        "float");
    auto ampMin = new ValueArg<float>("",
                                      "amp-min",
                                      "Set the lower amplification factor.",
                                      false,
                                      defaultFloats["amp-min"],
                                      "float");
    auto ampMax = new ValueArg<float>("",
                                      "amp-max",
                                      "Set the upper amplification factor.",
                                      false,
                                      defaultFloats["amp-max"],
                                      "float");
    auto ampCount = new ValueArg<int>("",
                                      "amp-count",
                                      "Set the number of amplification factors.",
                                      false,
                                      defaultInts["amp-count"],
                                      "integer");
...
{
    auto OutputFile =
        new ValueArg<string>("o",
                             "output",
                             "The basename of the output file. \"_[simtype].csv\" will "
