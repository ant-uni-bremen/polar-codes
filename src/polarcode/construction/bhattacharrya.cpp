...
Bhattacharrya::Bhattacharrya() : mInitialParameter(0.5) {}
...
void Bhattacharrya::setParameterByDesignSNR(float designSNR)
{
    ...
    mInitialParameter = exp(-2.0 * linearDesignSNR * mInformationLength / mBlockLength);
}
...
std::vector<unsigned> Bhattacharrya::construct()
{
    ...
    for (unsigned channel = 0; channel < (mBlockLength - mInformationLength); ++channel) {
