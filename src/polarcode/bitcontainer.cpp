 ...
 */
inline float frestrict(float min, float x, float max) { return fmin(fmax(x, min), max); }
...
BitContainer::BitContainer()
    : mElementCount(0), mFrozenBits({}), mInformationBitCount(0), mLUT(nullptr)
{
}

BitContainer::BitContainer(size_t size)
    : mElementCount(size), mFrozenBits({}), mInformationBitCount(size), mLUT(nullptr)
{
}

BitContainer::BitContainer(size_t size, const std::vector<unsigned>& frozenBits)
    : mElementCount(size), mInformationBitCount(size - frozenBits.size()), mLUT(nullptr)
{
    mFrozenBits.assign(frozenBits.begin(), frozenBits.end());
    calculateLUT();
}

BitContainer::~BitContainer() { clear(); }
...
size_t BitContainer::size() { return mElementCount; }
...
FloatContainer::FloatContainer() : mData(nullptr), mDataIsExternal(false)
{
...
void FloatContainer::setSize(size_t newSize)
{
    ...
    // Allocate new memory
    mData =
        static_cast<float*>(_mm_malloc(4 * mElementCount, 32 /*BYTESPER(AVX)VECTOR*/));
    if (mData == nullptr) {
...
void FloatContainer::getPackedInformationBits(void* pData)
{
    if (mInformationBitCount % 8 == 0) {
        ...
        for (unsigned byte = 0; byte < informationByteCount; byte++) {
            ...
            vec = _mm256_i32gather_epi32(
                cBit, lut, 4); // collect info LLRs from positions given by lookup table
...
float* FloatContainer::data() { return mData; }


CharContainer::CharContainer() : mData(nullptr), mDataIsExternal(false) {}
...
void CharContainer::insertLlr(const char* pLlr) { memcpy(mData, pLlr, mElementCount); }
...
char* CharContainer::data() { return mData; }


PackedContainer::PackedContainer()
    : mData(nullptr), mInformationMask(nullptr), mDataIsExternal(false)
{
}
...
PackedContainer::PackedContainer(char* external,
                                 size_t size,
...
void PackedContainer::buildInformationMask()
{
    ...
    for (unsigned i = begin; i < mFakeSize / 64; ++i) {
        ...
        // clear frozen bits
        while (frozenCounter < frozenBitCount &&
               mFrozenBits[frozenCounter] <
                   (i - begin + 1) * 64) { // while the next frozen bit is in this qword
            unsigned long bit = mFrozenBits[frozenCounter++] % 64;
            bit = (bit & ~7ULL) + (7 - (bit % 8)) +
                  bitOffset;       // pay attention to endianness
...
Problem:
    the pdep function works from low bit to high bit,
        but bit insertion is expected to work from high to
            low.Otherwise this code would work nicely.
    ...
    for (unsigned i = 0; i < mFakeSize / 64; ++i) {
        ...
        while (bitCount > availableBitCount) {
            ...
            unsigned long bitsAvailableInThatWord =
                (startBit > availableBitCount)
                    ? // check, if this word has enough new bits
                    64 - startBit
                    : // if not, load all it provides
                    64 - startBit -
                        availableBitCount; // if yes, load only the needed amount
            // load the bit providing qword
            unsigned long resourceWord = __bswap_64(uliData[nextWordToLoadFrom]);
            // extract the needed bits
            unsigned long freshBits =
                _bextr_u64(resourceWord, startBit, bitsAvailableInThatWord);
...
char* PackedContainer::data() { return mData; }
