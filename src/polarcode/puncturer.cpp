...
std::vector<unsigned> inverse_set_difference(size_t blockLength,
    ...
    std::set_difference(v.begin(), v.end(), positions.begin(), positions.end(),
	...
	mBlockLength(blockLength)
{
	...
	if(numPuncturedPos > frozenBitPositions.size()){
		throw std::out_of_range("Number of required puncturing positions exceeds frozen bit positions!");
	}
