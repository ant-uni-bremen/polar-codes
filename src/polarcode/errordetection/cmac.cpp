...
{
    unsigned char t_cmac[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
size_t cmac::calculate_cmac(unsigned char* cmac,
                            const unsigned char* key,
                            const unsigned int key_len,
                            const unsigned char* message,
                            const unsigned int msg_len)
{
    ...
    CMAC calculation via the standard EVP_Sign* interface is only supported in
    OpenSSL 1.1.0 and up. We use OpenSSL 1.0.1f at the moment.

    EVP_PKEY* pkey = NULL;
    EVP_PKEY_CTX* kctx = EVP_PKEY_CTX_new_id(EVP_PKEY_CMAC, NULL);
    EVP_PKEY_keygen_init(kctx);

    EVP_PKEY_CTX_ctrl(kctx, -1, EVP_PKEY_OP_KEYGEN, EVP_PKEY_CTRL_CIPHER, 0, (void*)
    EVP_aes_128_cbc()); EVP_PKEY_CTX_ctrl(kctx, -1, EVP_PKEY_OP_KEYGEN,
    EVP_PKEY_CTRL_SET_MAC_KEY, key_len, (void*) key); EVP_PKEY_keygen(kctx, &pkey);
