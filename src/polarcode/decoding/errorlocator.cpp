...
void Bit::decode() {
	mValue = *mOutput = mReplace ? mDesiredValue : *mInput;
}
...
void ErrorLocator::initialize(unsigned blockLength, const std::vector<unsigned> &frozenBits) {
...
void ErrorLocator::setAsReferenceDecoder() {
	mIsReferenceDecoder = true;
}
...
inline bool signEq(HybridFloat a, HybridFloat b) {
	return !((a.u ^ b.u) & 0x80000000U);
}
...
int ErrorLocator::firstError() {
	return mFirstError;
}

int ErrorLocator::correctionCount() {
