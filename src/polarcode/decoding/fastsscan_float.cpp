...
void Node::reset() { memFloatFill(mExt->data, 0.0f, mBlockLength); }
...
RateZeroNode::RateZeroNode(Node* parent) : Node(parent) {}

RateZeroNode::~RateZeroNode() {}

void RateZeroNode::reset() { memFloatFill(mOutput, INFINITY, mBlockLength); }

/*************
 * RateOneNode
 * ***********/

RateOneNode::RateOneNode(Node* parent) : Node(parent) {}

RateOneNode::~RateOneNode() {}

void RateOneNode::reset() { memFloatFill(mOutput, 0.0f, mBlockLength); }

/*************
 * RepetitionNode
 * ***********/

RepetitionNode::RepetitionNode(Node* parent) : Node(parent) {}

RepetitionNode::~RepetitionNode() {}

void RepetitionNode::reset() { memFloatFill(mOutput, 0.0f, mBlockLength); }
...
TwoBitNode::TwoBitNode(Node* parent) : Node(parent) {}
...
FastSscanFloat::FastSscanFloat(unsigned blockLength,
                               unsigned trialLimit,
...
FastSscanFloat::~FastSscanFloat() { clear(); }
...
void FastSscanFloat::initialize(unsigned blockLength,
                                const std::vector<unsigned>& frozenBits)
{
...
void FastSscanFloat::calculateOutput()
{
    FastSscanObjects::addVectors(
        mNodeBase->input(), mNodeBase->output(), mTemp->data, mBlockLength);
