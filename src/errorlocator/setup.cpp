...
void Configurator::setupArgumentWorkload() {
	auto workload = new ValueArg<long>("w", "workload", "Set the number of blocks to examine", false, defaultLongInts["workload"], "bits");
	insertArgument(workload);
}

void Configurator::setupArgumentSnr() {
	auto snr = new ValueArg<float>("s", "snr", "Set the SNR (Eb/No) in dB.", false, defaultFloats["snr"], "float");
	insertArgument(snr);
}


void Configurator::setupArgumentDesignSnr() {
	auto dsnrFixed = new ValueArg<float>("d", "design-snr", "Set the design-SNR parameter for Bhattacharrya code construction", false, defaultFloats["dsnr"], "float");
	insertArgument(dsnrFixed);
}

void Configurator::setupArgumentBlockLength() {
	auto nFixed = new ValueArg<int>("n", "blocklength", "Length of a Polar Code block.", false, defaultInts["n"], "int");
	insertArgument(nFixed);
}

void Configurator::setupArgumentCoderate() {
	auto rFixed = new ValueArg<float>("r", "rate", "Set the code rate.", false, defaultFloats["r"], "float");
