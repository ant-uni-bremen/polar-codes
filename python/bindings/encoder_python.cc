...
void bind_encoder(py::module &m)
{
    ...
    py::class_<ButterflyFipPacked>(m, "PolarEncoder")
        ...
        .def(
            "setErrorDetection", [](ButterflyFipPacked &self, unsigned size, std::string type) {
                self.setErrorDetection(PolarCode::ErrorDetection::create(size, type));
            },
            py::arg("size") = 0, py::arg("type") = "crc")
        .def("encode_vector", [](ButterflyFipPacked &self, const py::array_t<uint8_t, py::array::c_style | py::array::forcecast> &array) {
