...
template <unsigned subBlockLength>
inline void G_function(float LLRin[subBlockLength * 2],
                       float LLRout[subBlockLength],
                       float BitsIn[subBlockLength * 2])
{
    if (subBlockLength < 8) {
        for (unsigned i = 0; i < subBlockLength; ++i) {
            float& Left = LLRin[i];
...
class TemplatizedFloat : public Decoder
{
    ...
    inline void decodeNode(float input[size], float output[size])
    {
        ...
        if (frozenBitCount == size) {
            ...
            constexpr int leftFrozenBitCount =
                partialSum<begin, size / 2, N>(frozenBitSet);
            constexpr int rightFrozenBitCount =
                partialSum<begin + size / 2, size / 2, N>(frozenBitSet);
...
public:
    ...
    bool decode()
    {
        decodeNode<0, N>(dynamic_cast<FloatContainer*>(mLlrContainer)->data(),
