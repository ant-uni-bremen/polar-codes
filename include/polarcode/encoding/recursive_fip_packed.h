...
/*!
 * \brief For codes shorter than vector-length, falling back to butterfly encoder is
 * easier.
 */
...
public:
    ShortButterflyNode(std::vector<unsigned>& frozenBits,
                       Node* parent); ///< Initialize the butterfly encoder.
    ~ShortButterflyNode();
...
public:
    RateRNode(std::vector<unsigned>& frozenBits,
              Node* parent); ///< Initialize the general recursive encoder.
    ~RateRNode();
...
{
    RecursiveFip::Node *mNodeBase, *mRootNode;
    void clear();
...
public:
     ...
     */
    RecursiveFipPacked(size_t blockLength, const std::vector<unsigned>& frozenBits);
