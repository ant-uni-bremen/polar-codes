...
void MainWindow::updateTree()
{
    ...
    for (int lev = n - 1; lev >= 0; --lev) {
        ...
        for (idx = st, ctr = ed; idx < ed; ++idx) {
            ...
            } else if ((Left == RateHalf || Left == SPCnode) &&
                       Right == RateOne /* && lev >= n-2*/) {
                simplifiedTree[idx] = SPCnode;
            } else if (Left == RateZero &&
                       (Right == RateHalf || Right == RepetitionNode)) {
...
void MainWindow::updateScene()
{
    ...
    for (int layer = 0; layer <= n; ++layer) {
        ...
        if (layer == n)
            string += QStringLiteral(" bit");
        else
            string += QStringLiteral(" bits");
...
void MainWindow::addNodes(int nodeID, int layer, int element, bool gray)
{
    ...
    QRectF leftEllipse(leftCoord, ellipseSize), rightEllipse(rightCoord, ellipseSize);
...
QBrush MainWindow::getBrush(int nodeID) { return getBrushByCode(simplifiedTree[nodeID]); }
