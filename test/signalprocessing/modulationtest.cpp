...
void ModulationTest::testAsk() {
	SignalProcessing::Modulation::Modem *Modem = new SignalProcessing::Modulation::Ask(2, false);
	...
	bool modulationSuccess =
				 modulationOutput->at(0) == 3
			  && modulationOutput->at(1) == 1
			  && modulationOutput->at(2) == -3
			  && modulationOutput->at(3) == -1;

	CPPUNIT_ASSERT(modulationSuccess);

	demodulationInput.resize(modulationOutput->size());
	std::copy(modulationOutput->begin(), modulationOutput->end(), demodulationInput.begin());
